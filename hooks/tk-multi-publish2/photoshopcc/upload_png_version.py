...
class PhotoshopUploadPNGVersionPlugin(HookBaseClass):
    """
    ...
    def validate(self, settings, item):
        ...
        """
        self.logger.debug("----upload PNG version----")
        document = item.parent.properties["document"]
        publisher = self.parent
        path = _document_path(document)
        file_info = publisher.util.get_file_path_components(path)
        publish_path = item.properties.get("publish_png_path")
    ...
    def publish(self, settings, item):
        ...
        path_components = publisher.util.get_file_path_components(publish_path)
