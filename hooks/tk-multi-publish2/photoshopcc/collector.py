...
class PhotoshopCCSceneCollector(HookBaseClass):
    ...
    def process_current_session(self, settings, parent_item):
        ...
        if work_template:
            ...
            self._collect_png(document_item)
            self._collect_512_png(document_item)
            self._collect_2k_png(document_item)
            return
    ...
    def _collect_png(self,parent_item):
        '''
            photoshop png publish
            creator:huangsheng
        :param parent_item:
        :return:
        '''
        self.logger.debug("PNG publish...")
        publisher = self.parent
        engine = publisher.engine
        current_document = engine.adobe.get_active_document()
        ...
        png_ext_name = current_document.name.replace('.psd','.png')
        png_item = parent_item.create_item(
        ...
        png_item.set_icon_from_path(icon_path)

        # store the camera name so that any attached plugin knows which
        # camera this item represents!
        import re
        com = re.compile("([a-zA-Z].+)_")
        png_name =  com.findall(png_ext_name)
        if not png_name:
            error_msg = "PNG name dose not exists!"
            self.logger.error(error_msg)
            raise Exception(error_msg)
        png_item.properties["png_name"] = png_name[0]
        # set checked and expanded
        png_item._expanded = False
        png_item._active = False

    def _collect_512_png(self, parent_item):
        '''
            photoshop png 512 publish
            creator:huangsheng
        :param parent_item:
        :return:
        '''
    ...
    def _collect_2k_png(self, parent_item):
        '''
            photoshop png 512 publish
            creator:huangsheng
        :param parent_item:
